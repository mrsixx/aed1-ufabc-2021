...
int main()
{
    ...
    while(scanf("\n%s", str) != EOF)
    {
        ...
        {
            inserir(arvore, criaNo());
        }
        else if(strcmp(str, "delete") == 0)
        {
            ret = scanf("%s", word);
            no = remover(arvore, word);
            if(no != NULL)
            {
                //printf("%d\n", no->chave);
                free(no);
        ...
        else if(strcmp(str, "pre-order") == 0)
        {
            preOrder(arvore->raiz);
            printf("\n");
        }
        else if(strcmp(str, "in-order") == 0)
        {
            inOrder(arvore->raiz);
            printf("\n");
        }
        else if(strcmp(str, "post-order") == 0)
        {
...
struct No * criaNo()
{
    ...
    if(no != NULL)
    {
        ...
        no->direita = NULL;
        no->pai = NULL;
        no->chave = getChave(no->palavra);
    }
    return no;
}
int getChave(char *palavra)
{
    int chave = 0;
    for(int i = 0; i < 55 && palavra[i] != '\0' && palavra[i] != '\n'; i++)
        chave += (int)palavra[i];
